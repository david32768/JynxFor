...
public class JynxCodeNodeBuilder {
    ...
    public void addInstruction(JynxInstruction insn) {
        instructionList.add(new JynxInstructionNode(insn));
    }
    
    public void addInstructionList(List<JynxInstruction> insnlist) {
        for (var inst : insnlist) {
            addInstruction(inst);
        }
    }
    ...
    public void addFrameToLabel(Object[] stackArray, Object[] localArray) {
        if (!instructionList.isEmpty() && instructionList.getLast().instruction() instanceof LabelInstruction labinst) {
    ...
    public JynxCodeNode build(StackLocals stacklocals) {
        ...
        boolean hasop = instructionList.stream()
                .map(inst -> inst.instruction().jvmop())
        ...
        if (!(instructionList.getFirst().instruction() instanceof LabelInstruction)) {
            JynxLabel label = new JynxLabel(":start");
            LabelInstruction labinst = new LabelInstruction(JvmOp.xxx_label, label, Line.GENERATED);
            instructionList.addFirst(new JynxInstructionNode(labinst));
        }
        if (!(instructionList.getLast().instruction() instanceof LabelInstruction)) {
            JynxLabel label = new JynxLabel(":end");
            LabelInstruction labinst = new LabelInstruction(JvmOp.xxx_label, label, Line.GENERATED);
            instructionList.addLast(new JynxInstructionNode(labinst));            
        }
